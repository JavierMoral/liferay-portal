...
public class DuplicatePageElementExceptionProblemMapper
	...
	public Problem getProblem(
		...
		String externalReferenceCode =
			duplicatePageElementException.getExternalReferenceCode();

		String message =
			"A page element with the same external reference code already " +
				"exists";

		if (Validator.isNotNull(externalReferenceCode)) {
			message = StringBundler.concat(
				"A page element with external reference code ",
				externalReferenceCode, " already exists");
		}

		return ProblemUtil.getProblem(
			message, Problem.Status.CONFLICT, duplicatePageElementException);
	}
